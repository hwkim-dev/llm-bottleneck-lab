    ...
    try:
        ...
        W_embed, W_ple_packed, W_ple_scale, norm_ple, W_ple_proj, altup_projs, altup_unprojs, \
            W_final_norm, W_lm_head, W = safeTensor.load_local_weights(
                cache_embeddings=ENGINE_CONFIG.get("cache_embeddings", False))
...
def status():
    return jsonify({
        ...
        "model": "Gemma 3N E4B",
        "weight_mode": (safeTensor.LOADED_MODE or gem_main.WEIGHT_MODE).upper(),
        "accel": gem_main.ACCEL_MODE,
...
def chat():
    ...
    def generate():
        ...
        if ENGINE_CONFIG.get("use_spec_decode"):
            ...
            for kind, payload in spec_decode.generate_speculative(
                ...
                if kind == "token":
                    ...
                    yield f"data: {json.dumps({'token': new_text, 'count': count})}\n\n"
                else:
        ...
        yield f"data: {json.dumps({'done': True, 'total_tokens': token_count, 'context_pos': cur_pos, 'stopped': stopped_by_user})}\n\n"
...
def chat_stop():
    """Signal the active generation loop to break out after the current token."""
...
if __name__ == "__main__":
    init_model()
    print("\n" + "=" * 50)
    print("  Gemma 3N E4B  |  Flask backend ready at http://127.0.0.1:5000")
    print("=" * 50 + "\n")
