...
# ─── 5. Native GUI (CMake) ───────────────────────────────────
hdr "5/6  Building Native GUI (Vulkan + ImGui)"

BUILD_NATIVE=true
if ! command -v cmake &>/dev/null; then
    warn "cmake not found — skipping native GUI build"
    BUILD_NATIVE=false
fi

if $IS_RPI; then
    warn "Raspberry Pi: native GUI build is experimental (VideoCore VI Vulkan)"
    warn "  Skipping by default. Set BUILD_NATIVE_RPI=1 to force."
    [[ "${BUILD_NATIVE_RPI:-0}" != "1" ]] && BUILD_NATIVE=false
fi

if $BUILD_NATIVE && [[ -d "$NATIVE_DIR" ]]; then
    ...
    warn "Skipping native GUI build"
fi
...
echo -e "${W}══════════════════════════════════════════${N}"
echo ""
echo -e "  Web GUI:     ${C}bash run.sh${N}"
if [[ -f "$NATIVE_DIR/build/llm-lite-native" ]]; then
echo -e "  Native GUI:  ${C}bash run-native.sh${N}"
fi
echo ""
if ! ls "$WEIGHTS_DIR"/*.safetensors &>/dev/null 2>&1 && \
   ! ls "$WEIGHTS_DIR"/*.npy &>/dev/null 2>&1; then
